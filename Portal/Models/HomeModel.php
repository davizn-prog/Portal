...
class HomeModel
{
	...
	public static function postFeed($post, $tipo, $nomeArquivo)
	{
		...
		$postFeed->execute(array($_SESSION['id'], $post, date('Y-m-d H:i:s', time()), $tipo, $nomeArquivo));
	...
	public static function retrieveFriendsPosts()
	{
		...
		foreach ($amigosConfirmados as $key => $value) {
			...
			// $listaAmigos[$key]['tipo'] = \Portal\Models\UsuariosModel::getUsuarioById($value)['tipo'];
			// $listaAmigos[$key]['arquivo_url'] = \Portal\Models\UsuariosModel::getUsuarioById($value)['arquivo_url'];
		}
